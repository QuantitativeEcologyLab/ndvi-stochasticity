...
Together, the sets of models for mean NDVI ($\mu$) and and variance in NDVI ($\sigma^2$) had the following structure:
\begin{equation} \label{eq:model}
\begin{cases}
\text{NDVI} \sim \text{Normal}(\mu, \sigma^2) \\
\\
\begin{aligned}
\mu = \beta_{\mu} + s(\texttt{prop\_water}) + s(\texttt{elev\_m}) + s(\texttt{doy}) + s(\texttt{year}) + s(\texttt{lat}, \texttt{long}) + \\
t(\texttt{doy}, \texttt{year}) + t(\texttt{doy}, \texttt{lat}, \texttt{long}) + t(\texttt{year}, \texttt{lat}, \texttt{long})
\end{aligned} \\
\\
\begin{aligned}
\sigma^2 = \beta_{\sigma^2} + s(\texttt{prop\_water}) + s(\texttt{elev\_m}) + s(\texttt{doy}) + s(\texttt{year}) + s(\texttt{lat}, \texttt{long}) + \\
t(\texttt{doy}, \texttt{year}) + t(\texttt{doy}, \texttt{lat}, \texttt{long}) + t(\texttt{year}, \texttt{lat}, \texttt{long}) + s(\mu)
\end{aligned}
\end{cases},
\end{equation}

\noindent where, for a given (spatiotemporally aggregated) pixel, \texttt{prop\_water} is the proportion of water within the pixel, \texttt{elev\_m} is elevation above sea level in meters, \texttt{doy} is integer day of year (1-366), \texttt{year} is integer year (1981-2025), \texttt{lat} and \texttt{long} are latitude and longitude, respectively. For ease of readability, we omit the pixel-level indices. $s(~)$ indicates marginal smooth functions, while $t(~)$ indicates tensor product interaction terms. Seasonal terms used cyclical cubic splines (\texttt{bs = `cc'}) with edge knots 0.5 and 366.5, while spatial smooths used a basis with second-order spline on the sphere, computed by Wendelberger's \citep{wendelberger_smoothing_1982} method \citep[\texttt{bs = `sos'}; ][]{wood_generalized_2017}. The linear predictors for $\mu$ and $\sigma^2$ have the same structure, other than the $f_{9}(\mu)$ smooth term for $\sigma^2$, but the functions for $\sigma^2$ are independent of those for $\mu$; we use the same in. Despite the data aggregation procedures and faster fitting methods, including parallelizing computation across 60 cores, each of the four models required multiple days to fit.

The residuals from each of the four models served to estimate the spatiotemporal trends in the variance. However, the discretized predictions from models \ref{eq:model} were excessively coarse over space, so predictions were calculated without discrete prediction methods (\texttt{discrete = FALSE}), at the cost of substantially longer computational times. A second set of GAMs estimated the variance as the mean squared residual across time and space using the model structure as above and an additional smooth term of the estimated mean NDVI:

\noindent where smooth terms are defined as with model \ref{eq:model} but are independent from it, and $f_{9}(\mu)$ indicates a smooth term of mean NDVI for a given location in space and date in time. It is worth noting that, since there are no corrections to any bias in residuals from models \ref{eq:model}, the estimates of variance from models \ref{eq:model} may be inflated near areas or periods of rapid and substantial change. Still, the appreciability of this inflation is dependent on the spatiotemporal scale over which changes are perceived \citep{levin_problem_1992,frankham_importance_2004,goodchild_scale_2023,mezzini_how_2025}.
